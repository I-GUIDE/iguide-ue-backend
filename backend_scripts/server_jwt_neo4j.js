...
// Middleware for JWT-specific CORS policy
// Middleware for JWT-specific CORS policy
const jwtCorsMiddleware = (req, res, next) => {
  const jwtProtectedRoutes = [
    { path: '/api/upload-avatar', method: 'POST' },
    { path: '/api/update-avatar', method: 'POST' },
    { path: '/api/upload-dataset', method: 'POST' },
    { path: '/api/upload-thumbnail', method: 'POST' },
    { path: '/api/element', method: 'POST' },
    { path: '/api/resources/', method: 'DELETE' },
    { path: '/api/users', method: 'POST' },
    { path: '/api/element/', method: 'PUT' },
    { path: '/api/users/', method: 'PUT' }
  ];

  const isJwtProtected = jwtProtectedRoutes.some(route =>
    req.path.startsWith(route.path) && req.method === route.method
  );

  if (isJwtProtected) {
    cors(jwtCorsOptions)(req, res, next);
  } else {
    next();
  }
};

app.use(jwtCorsMiddleware);
 ...
 */
app.post('/api/upload-avatar', authenticateJWT, uploadAvatar.single('file'), (req, res) => {
 ...
 */
app.post('/api/update-avatar', authenticateJWT, uploadAvatar.single('file'), async (req, res) => {
    // [Done] Neo4j
    try {
 ...
 */
app.post('/api/upload-dataset', authenticateJWT, upload.single('file'), (req, res) => {
    // [Done] Neo4j not required
    if (!req.file) {
 ...
 *         description: No file uploaded
 */
app.post('/api/upload-thumbnail', uploadThumbnail.single('file'), authenticateJWT, (req, res) => {
    if (!req.file) {
	return res.status(400).json({ message: 'No file uploaded' });
    }
 ...
 *         description: Internal server error
 */
app.post('/api/element', authenticateJWT, async (req, res) => {
    // [Done] Neo4j
    const resource = req.body;
 ...
 *         description: Internal server error
 */
app.put('/api/element/:id', authenticateJWT, async (req, res) => {
 ...
 *         description: Internal server error
 */
app.put('/api/users/:openid', authenticateJWT, async (req, res) => {
    const openid = decodeURIComponent(req.params.openid);
    const updates = req.body;
