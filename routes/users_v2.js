...
router.get('/api/users',
    ...
    try {
		...
		const response = await getAllContributorsV2(from, size, sort_by, sort_order, filter_key, filter_value);
...
router.get('/api/users/:id/valid', cors(), async (req, res) => {
    ...
    try {
		const response = await checkContributorByIDV2(id);
		res.status(200).json(response);
    } catch (error) {
...
router.post('/api/auth/users',
    ...
    try {
		...
		let existing_user = {}
		if (id !== undefined) {
			existing_user = await getContributorByIDv2(id);
		}
		if (existing_user !== {} && existing_user['id'] !== undefined) {
			...
			const response = await registerContributorAuthV2(user);
...
router.post('/api/users',
    ...
    try {
		const id = user['id'];
		const response = await registerContributorV2(user);
		if (response){
...
router.post('/api/users/merge',
	...
	try {
        ...
        // If any steps fail rollback to the original one so make sure to remove the alias relation after update
	} catch (error) {
